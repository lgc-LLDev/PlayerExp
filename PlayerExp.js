  ...
  const getItem = (award) => {
    if ('type' in award) {
      ...
      if (type === 'score') {
        ...
        if (!scoreObj) mc.runcmdEx(`scoreboard objectives add "${scoreName}" dummy`)
...
function levelUp(player, { lvl }) {
  mc.runcmdEx(`title "${player.realName}" subtitle §6Lv.${lvl - 1} §f--> §eLv.${lvl}`)
...
function sendAddXpTip(player, total) {
  ...
  const { exp, thisLevelExp, nextLevelExp, lvl } = getPlayerExpData(player.xuid)
...
mc.listen('onMobDie', (mob, source) => {
  ...
  for (const expSource of killSources) {
    if (expSource.value.includes(mob.type)) total += addPlayerExp(player, expSource.exp)
  }
